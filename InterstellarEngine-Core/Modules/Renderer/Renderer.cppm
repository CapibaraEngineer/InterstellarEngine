...
export namespace interstellarEngineCore::Renderer {
	...
	private:
		...
		void createTextureImage() {
			...
			std::string pathString = modelTexturePath.string();
			std::function<void* (assetManager::assetEntry&)> loadFunc = assetManager::assetDB.at(pathString).func;
			void* textureVoidP = loadFunc(assetManager::assetDB.at(pathString));
			fileTypes::imageFile* texturePointer = static_cast<fileTypes::imageFile*>(textureVoidP);
			int texWidth = texturePointer->width;
			int texHeight = texturePointer->height;
			int texChannels = texturePointer->channels;
			VkDeviceSize imageSize = texWidth * texHeight * 4;
			mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;
			...
			void* data;
			vkMapMemory(device, stagingBufferMemory, 0, imageSize, 0, &data);	
			memcpy(data, texturePointer->pixelData.get(), static_cast<size_t>(imageSize));
			vkUnmapMemory(device, stagingBufferMemory);
